        ...
        if ( $instance['extra_posts'] && $instance['extra_num'] ) {
            ...
            if ( have_posts ( ) ) :
                ...
                elseif ( strlen( $listitems ) > 0 ) {
                    printf( '<select id="%s" value="%s" onchange="window.location=this.options[this.selectedIndex].value"><option value="none">%s %s</option>%s</select>', $this->get_field_id( 'extra_format' ), get_permalink(), __( 'Select', 'gfwa' ), $instance['post_type'], $listitems );
                }
